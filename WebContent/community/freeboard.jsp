...
<%
	...
	try {
		...
		pstmt = conn.prepareStatement("SELECT COUNT(NUM) AS TOTAL FROM freeboard WHERE ISDELETE != 1" + whereSQL);
		if (!"".equals(whereSQL)) {
			if ("ALL".equals(searchType)) {
		...
		pstmt = conn.prepareStatement("SELECT NUM, TITLE, WRITER, REG_DATE, HIT FROM freeboard WHERE ISDELETE != 1"+whereSQL+" ORDER BY NUM DESC LIMIT ?, ?");
		if (!"".equals(whereSQL)) {
		...
		<contents>
			<div class='contentsTotal'>
				<div class='contentsLink'>
					<a href='../home.jsp'>홈</a>><a href='community.jsp'>커뮤니티</a>><a href='freeboard.jsp'>자유게시판</a>
				</div>
				...
				<div class='contentsBox'>
						...
						<tbody>
							...
							<tr>
				 			<td align="center"><%=totalCount - i + 1 - (pageNumTemp - 1) * listCount %></td>
								<td style="text-indent:5px;"><a href="freeboardView.jsp?num=<%=rs.getInt("NUM")%>&amp;pageNum=<%=pageNumTemp%>&amp;searchType=<%=searchType%>&amp;searchText=<%=searchText%>"><%=rs.getString("TITLE") %></a></td>
					...
					<p  align="right">
						<input type="button" class='small secondary button'value="글쓰기" onclick="goUrl('freeboardWriteForm.jsp');" />
					</p>
						...
						<td style="text-align:center; background:#ffffff;">
							<%
										if(totalCount > 0) {
											...
											if(pageNumTemp > 1){
												sb.append("<a href=\"").append("freeboard.jsp?pageNum=1&amp;searchType="+searchType+"&amp;searchText="+searchText);
												sb.append("\" title=\"<<\"><<</a>&nbsp;");
											}
											if (isPrev) {
												int goPrevPage = startPage - pagePerBlock;			
												sb.append("&nbsp;&nbsp;<a href=\"").append("freeboard.jsp?pageNum="+goPrevPage+"&amp;searchType="+searchType+"&amp;searchText="+searchText);
												sb.append("\" title=\"<\"><</a>");
											} else {
											...
											for (int i = startPage; i <= endPage; i++) {
												if (i == pageNumTemp) {
													...
													sb.append("<a href=\"").append("freeboard.jsp?pageNum="+i+"&amp;searchType="+searchType+"&amp;searchText="+searchText);
											...
											if (isNext) {
												...
												sb.append("<a href=\"").append("freeboard.jsp?pageNum="+goNextPage+"&amp;searchType="+searchType+"&amp;searchText="+searchText);
												sb.append("\" title=\">\">></a>");
											} else {
												
											}
											if(totalNumOfPage > pageNumTemp){
												sb.append("&nbsp;&nbsp;<a href=\"").append("freeboard.jsp?pageNum="+totalNumOfPage+"&amp;searchType="+searchType+"&amp;searchText="+searchText);
												sb.append("\" title=\">>\">>></a>");
											}
					...
					<tr>
						<td style="background:#ffffff; ">
							<form name="searchForm" action="freeboard.jsp" method="get" onsubmit="return searchCheck();">
