...
Connection conn = null;
	...
	try {
		...
		pstmt = conn.prepareStatement("UPDATE freeboard SET HIT = HIT + 1 WHERE NUM = ?");
			...
			<div class='contentsTotal'>
				...
				<div class='contentsBox'>
					...
					<p class="btn_align" align="right">
						<input type="button" class='small secondary button' value="목록" onclick="goUrl('freeboard.jsp?pageNum=<%=pageNum%>&amp;searchType=<%=searchType%>&amp;searchText=<%=searchText%>');" />
						<input type="button" class='small secondary button' value="수정" onclick="goUrl('freeboardModifyForm.jsp?num=<%=num%>&amp;pageNum=<%=pageNum%>&amp;searchType=<%=searchType%>&amp;searchText=<%=searchText%>');" />
						<input type="button" class='small secondary button' value="삭제" onclick="deleteCheck('boardProcess.jsp?mode=freeD&amp;num=<%=num%>&amp;pageNum=<%=pageNum%>&amp;searchType=<%=searchType%>&amp;searchText=<%=searchText%>');" />
					</p>
				</div>
			</div>
		</contents>
