...
<%
	...
	try {
		...
		pstmt = conn.prepareStatement("SELECT COUNT(NUM) AS TOTAL FROM newsboard WHERE ISDELETE != 1" + whereSQL);
		if (!"".equals(whereSQL)) {
			if ("ALL".equals(searchType)) {
		...
		pstmt = conn.prepareStatement("SELECT NUM, TITLE, WRITER, REG_DATE, HIT FROM newsboard WHERE ISDELETE != 1"+whereSQL+" ORDER BY NUM DESC LIMIT ?, ?");
		if (!"".equals(whereSQL)) {
		...
		<contents>
			<div class='contentsTotal'>
				...
				<div class='contentsBox'>
						...
						<tbody>
							...
							<tr>
				 			<td align="center"><%=totalCount - i + 1 - (pageNumTemp - 1) * listCount %></td>
								<td style="text-indent:5px;"><a href="newsboardView.jsp?num=<%=rs.getInt("NUM")%>&amp;pageNum=<%=pageNumTemp%>&amp;searchType=<%=searchType%>&amp;searchText=<%=searchText%>"><%=rs.getString("TITLE") %></a></td>
					...
					%>					
					<p  align="right">
						<input type="button" class='small secondary button'value="글쓰기" onclick="goUrl('newsboardWriteForm.jsp');" />
					</p>
					<%
					}
					%>
						...
						<td style="text-align:center; background:#ffffff;">
							<%
										if(totalCount > 0) {
											...
											if(pageNumTemp > 1){
												sb.append("<a href=\"").append("newsboard.jsp?pageNum=1&amp;searchType="+searchType+"&amp;searchText="+searchText);
												sb.append("\" title=\"<<\"><<</a>&nbsp;");
											}
											if (isPrev) {
												int goPrevPage = startPage - pagePerBlock;			
												sb.append("&nbsp;&nbsp;<a href=\"").append("newsboard.jsp?pageNum="+goPrevPage+"&amp;searchType="+searchType+"&amp;searchText="+searchText);
												sb.append("\" title=\"<\"><</a>");
											} else {
											...
											for (int i = startPage; i <= endPage; i++) {
												if (i == pageNumTemp) {
													...
													sb.append("<a href=\"").append("newsboard.jsp?pageNum="+i+"&amp;searchType="+searchType+"&amp;searchText="+searchText);
											...
											if (isNext) {
												...
												sb.append("<a href=\"").append("newsboard.jsp?pageNum="+goNextPage+"&amp;searchType="+searchType+"&amp;searchText="+searchText);
												sb.append("\" title=\">\">></a>");
											} else {
												
											}
											if(totalNumOfPage > pageNumTemp){
												sb.append("&nbsp;&nbsp;<a href=\"").append("newsboard.jsp?pageNum="+totalNumOfPage+"&amp;searchType="+searchType+"&amp;searchText="+searchText);
												sb.append("\" title=\">>\">>></a>");
											}
					...
					<tr>
						<td style="background:#ffffff; ">
							<form name="searchForm" action="newsboard.jsp" method="get" onsubmit="return searchCheck();">
