...
Connection conn = null;
	...
	try {
		...
		conn = DriverManager.getConnection(
			"jdbc:mysql://localhost:3306/code1","root","lee4874");
		
		pstmt = conn.prepareStatement(
			"SELECT NUM, TITLE, CONTENTS, WRITER, HIT, REG_DATE FROM noticeboard "+ 
			"WHERE NUM = ?");
...
<head>
	...
	<script type="text/javascript">
		...
		function boardModifyCheck() {
			var form = document.noticeboardModifyForm;
			if (form.title.value == '') {
				alert('제목을 입력하세요.');
				form.title.focus();
				return false;
		...
		<contents>
			<div class='contentsTotal'>
				...
				<div class='contentsBox'>
						<form name="noticeboardModifyForm" action="boardProcess.jsp" method="post" onsubmit="return boardModifyCheck();">
						<input type="hidden" name="mode" value="noticeM" />
						<input type="hidden" name="num" value="<%=num %>" />
						<input type="hidden" name="pageNum" value="<%=pageNum %>" />
						<input type="hidden" name="searchType" value="<%=searchType %>" />
						<input type="hidden" name="searchText" value="<%=searchText %>" />
						<table border="1" summary="글 수정" width="600px">
							<tr>
								<td width="150px" height="45px" align="center">
									<p class="box"><span style="font-size: 19px;">글 제목</span></p>  
								</td>
								<td>
									<input type="text" name="title" size="65" maxlength="100" value="<%=rs.getString("TITLE") %>"/>
								</td>	
							</tr>
							<tr>
								...
								<td>
									<textarea name="contents"><%=rs.getString("CONTENTS") %></textarea>
									<script>
						...
						<p align="right" style="width:600px;">
							<input class='small secondary button' type="button" value="목록" onclick="goUrl('noticeboard.jsp?pageNum=<%=pageNum%>&amp;searchType=<%=searchType%>&amp;searchText=<%=searchText%>');" />
							<input class='small secondary button' type="submit" value="글수정" />
						</p>
						</form>
				</div>
			</div>
