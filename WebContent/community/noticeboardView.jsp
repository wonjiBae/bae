...
Connection conn = null;
	...
	try {
		...
		pstmt = conn.prepareStatement("UPDATE noticeboard SET HIT = HIT + 1 WHERE NUM = ?");
			...
			<div class='contentsTotal'>
				...
				<div class='contentsBox'>
					...
					<p class="btn_align" align="right">
						<input type="button" class='small secondary button' value="목록" onclick="goUrl('noticeboard.jsp?pageNum=<%=pageNum%>&amp;searchType=<%=searchType%>&amp;searchText=<%=searchText%>');" />
						<%
			  			if ((session.getAttribute("userid")).equals(rs.getString("WRITER"))) {
						%>						
						<input type="button" class='small secondary button' value="수정" onclick="goUrl('noticeboardModifyForm.jsp?num=<%=num%>&amp;pageNum=<%=pageNum%>&amp;searchType=<%=searchType%>&amp;searchText=<%=searchText%>');" />
						<input type="button" class='small secondary button' value="삭제" onclick="deleteCheck('boardProcess.jsp?mode=noticeD&amp;num=<%=num%>&amp;pageNum=<%=pageNum%>&amp;searchType=<%=searchType%>&amp;searchText=<%=searchText%>');" />
						<%
			  			}
						%>
					</p>
